...
- You must declare uses at the beginning of the code.
- You can't access private (not-exported) definitions.
:::

## Import Paths

Import paths are data used to import packages. It is written between double quotes, but they are not a string literal. The path is handled by removing quotes.
...
- Each subpackage is separated from the others by the `/` character. There cannot be an empty distinction, so `//` is invalid. Cannot start or end with `/` also.
...
└─ main.jule
```
As you can see in your project tree, your main package `head` directory has a `foo` directory. This directory is a subpackage accessible to you, and it also has a subpackage called `bar`. However, also check where your `jule.mod` file is located.
...
An automatic assignment is made based on the last identifier of the import path. This means that the last subpackage name is automatically considered for an alias.

For example, some import paths and their parts are used as aliases:
- `foo`: `foo`
- `bar/foo`: `foo`
- `foo/bar/baz`: `baz`
- `foo/_bar_`: `_bar_`
...
To achieve this, there is no keyword for this. Just provide an identifier before the import path.
