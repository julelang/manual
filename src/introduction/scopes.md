...
Defer scopes are technically not closures. They refer to all variables within the scope by reference. That is, they do not use copies; they modify the actual variables directly. Return values and use values are not affected by this. In other words, when you write `return x`, even if a defer scope that runs after the return statement changes the value of `x`, the return value does not change. There is one exception: if the return values themselves are variables, then the return value is affected.
