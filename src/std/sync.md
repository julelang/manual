...
This mutex implementation will not check ownership of the mutex\. So, a locked Mutex is not associated with a particular thread\. It is allowed for one thread to lock a Mutex and then arrange for another thread to unlock it\.
...
```
A WaitGroup waits for a collection of threads/tasks to finish\. The main thread calls \[WaitGroup\.Add\] to set the number of threads/tasks to wait for\. Then each of the threads/tasks runs and calls \[WaitGroup\.Done\] when finished\. At the same time, \[WaitGroup\.Wait\] can be used to block until all threads/tasks have finished\.
