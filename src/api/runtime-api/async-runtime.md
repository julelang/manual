...
For the `await` keyword on the Jule side, you can use the `__jule_AsyncAwait` macro, and for the `return` keyword, you can use the `__jule_AsyncReturn` macro.

Here is an example:
```cpp
__jule_VoidAsync checkdiv(__jule_Int z) {
	if (z == 0) {
		__jule_panicString("divide by zero");
	}
	__jule_AsyncReturn;
}
...
`__jule_AsyncReturn` marks the logical completion point of a Jule async function and is required to correctly finalize the underlying coroutine promise. Omitting it may cause the generated code to violate the expected async ABI and lead to undefined behavior at runtime.

Therefore, all control-flow paths of an async function must terminate with `__jule_AsyncReturn`.