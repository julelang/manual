...
These changes are promised to provide gains in various aspects (e.g., runtime performance).
...
Production compilation is a compiler feature that must be enabled separately using the `-p` or `--production` flag. It behaves differently from optimization options, which are enabled with `--opt` options.
...
When you enable production compilation, it provides improvements that optimization options cannot provide. It can cause some changes to the IR and additionally prompts your backend compiler to compile the IR code more efficiently. In this way, significantly faster programs can be obtained by using optimization options and production compilation together.

Production compilation also normalizes information to make ready-to-production. This information is embedded, which is added to the executable file for debugging reasons with minimum cost, such as path information of panic calls, etc. Normally, the path information is an absolute path, but in production compilation, absolute paths can be ambiguous and complex to read. To prevent this, production compilation removes the prefix of the absolute path and leaves just the necessary root directory and the following path. For example, if a panic call is raised from the standard library, the path starts with `std/`; if a panic call is raised from the main package, the paths start with `mainPackageDirectory/`.
