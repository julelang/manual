...
When slices are allocated with a `make` call, all allocated space is automatically initialized with the default value.
...
Since arrays are fixed-length structures, all their elements are initialized with the default value according to their length. In the case of any literal, elements initialized with a value are not initialized with a default value. If a literal does not initialize all elements of the array, the remaining elements are automatically initialized with the default value.
...
For example, if a function increases a static integer memory every time it is called, the relevant static variable is mutated as it will be called during each default initialization.
...
\
If the given smart pointer is nil, then the allocation pointer of the `any` will be nil. Will not point to the smart pointer with additional allocation. The `any` type always tries to use smart pointers as base allocation and shares the same memory.

- **Type Pointer**\
An `any` maintains a general pointer, and this pointer is not traced by the GC because it is guaranteed to always point to static memory that will be available for the lifetime of the program. This pointer points directly to the type handler structure automatically created by the compiler.\
\
The handler structure includes the deallocator function required for the type. The deallocator function is the first field of the structure. Also contains 3 function pointers for string conversion, comparison functions, and a hasher for the stored type.

#### Nil Value Handling

The `Allocation` section describes how allocation is handled for the `any` type. And it says nil smart pointer results as nil allocation pointer. But, this creates ambiguous behavior if the `any` equals nil when assigned with a nil smart pointer. It should point to a nil smart pointer.

Therefore, nil checking will use the type pointer data. So, if the type pointer is nil, then the `any` type is equal to nil. But if the type pointer is not nil, the `any` is considered as not nil, even with nil allocation data. Thus, we have pointed to the nil smart pointer behavior for nil smart pointers. No different behavior for some special cases.

### Type Enums

It is the same as the `any` type. They are implemented as `any` type by the compiler in the background. Type protection and restrictions are only at compile time. It doesn't have any additional runtime cost; it's just an `any` type.
...
Inheritance cost is relatively directly proportional to the number of implemented structures. However, this is an algorithm that performs the minimum effort required, so most of the time there is no need to worry about overhead.
...
Arrays are always allocated on the stack and have a fixed size. It does not have mutable memory between copies. In each copy, a new array with the same size is created, and the elements are copied according to the relevant type.

At runtime, an array stores and uses 1 different piece of data;
- **Array Pointer**\
A pointer to the first element of the array.

### Slicing

In arrays, the last index is limited to the length of the array. Slicing behavior is like slices. Arrays do not support direct slicing, and their sizes are fixed at compile time. Since the use of dynamic values ​​is allowed in slicing, the result of slicing is a slice, not an array. The returned slice will be allocated according to the relevant array range.
...
At runtime, a slice stores and uses 3 different types of data;

- **Main Allocation**\
It is a pointer shared among all slice copies and points to the main allocation of the slice. There is no guarantee that it will represent elements of the slice. The main usage is to protect and deallocate the original main allocation. It will always point to the base allocation.
...
- **Capacity**\
Capacity data of a slice of the main allocation.

- **Length**\
Length data of the slice of the main allocation.
...
Maps are structures implemented with Jule. They are implemented as smart pointers by the compiler in the background. This smart pointer is initialized with `nil` by default. The structure pointed to by the smart pointer is the map structure implemented by Jule. Various built-in functions are treated as calls to the methods of this structure.
...
Regardless of the backend language on which Jule will be compiled, strings will be length-based, not NULL-terminated, as in C. Jule strings are always implemented as length-based.

At runtime, a string stores and uses 3 different types of data;

- **Main Allocation**\
It is a pointer shared among all string copies and points to the main allocation of the string. There is no guarantee whether it will represent characters of a string. The main usage is to protect and deallocate the original main allocation. It will point to the base allocation.

- **Slice Pointer**\
It is a pointer that is guaranteed to point anywhere on the main allocation. It always points to the first element of the slice of a string.
...
- In some fields, it may be necessary to return byte-slice allocations as strings. In this case, if you are sure it is safe, you may want to unsafely convert it to a string to avoid a new allocation. But before doing this, you need to make sure that there is a NULL termination every time. Besides this, increasing the developer's considerations, perhaps that single NULL termination you add to the slice will, in some cases, exceed the capacity and result in a new allocation, in which case it's not much different than converting to a string.
...
Smart pointers are pointers that work depending on the GC technique used. They are basically implemented as a raw pointer wrapper, but are more safety restricted to be used with Safe Jule.

Some data types of Jule also use smart pointers in the background. This is because they reference each other in the space they allocate. This is why some types use background smart pointers to minimize the number of allocations. Therefore, they have additional overhead, such as the additional atomicity of smart pointers and the memory space allocated for reference counting.
...
Anonymous functions and closures are usually cheap. An anonymous function or closure is not much different from ordinary functions. They are implemented in the same way in the background; only the name is automatically chosen by the compiler.

Anonymous functions and closures are stored and moved with pointers, just like ordinary functions. An anonymous function or closure has no additional cost to call. For anonymous functions, there is no additional execution cost, unlike closures.

Closures impose some additional costs. One of these is captured variables. A heap allocation is created for captured variables and is guaranteed to be traced and deallocated by the GC. There is no risk of a memory leak. A closure is automatically deallocated when it is no longer reachable. There are no other memory footprints.
...
Due to the use of closures, all anonymously used functions have an additional hidden parameter. This hidden parameter is a pointer with a common type traced by the GC and is null for functions other than other closures. If an ordinary function is used as an anonymous function, the compiler adds this hidden parameter for it. This parameter is only required for closures and is handled only by closures; other functions will ignore it.

A few additional instructions are added to the closure to handle this parameter. These instructions are usually cheap and do not impose high runtime costs. The instructions usually include converting the hidden parameter to the closure's environment data.
...
Atomic operations are essential for references to be concurrency-safe, but in cases where this is not necessary? Atomicity means overhead, which means loss of performance. It is inherently unnecessary to have atomicity when atomicity is not required. Jule references works atomic because concurrency-safety must be guaranteed. This means that references will use atomicity for reference counting on each copy. This atomicity creates an atomicity overhead in memory with each copy operation.

The Jule compiler performs an automatic analysis to eliminate this burden. This analysis is based on the built-in concurrency of the language. If built-in concurrency is not used, the program implements reference counting without concurrency safety. Any use of concurrency triggers the concurrency-safe implementation for reference counting, and the cost of atomicity appears. This optimization saves the performance cost of atomicity for non-concurrent programs automatically.

## Termination of The Program

Jule behaves naively during program termination according to its own semantics. Although it provides good interoperability with C and C++, it does not guarantee fully correct coordination with them in all cases, such as ensuring that destructors are called during program shutdown.

When working with C or C++, you do not write code that is perfectly synchronized with Jule; you write code that is merely interoperable. Therefore, the Jule runtime does not guarantee that any C++ destructor functions will be executed or that anything else will happen on the C or C++ side during shutdown.